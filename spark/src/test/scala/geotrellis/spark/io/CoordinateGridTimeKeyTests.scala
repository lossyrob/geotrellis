...
trait CoordinateGridTimeKeyTests { self: PersistenceSpec[GridTimeKey, Tile, TileLayerMetadata[GridTimeKey]] =>
  ...
  for(PersistenceSpecDefinition(keyIndexMethodName, _, layerIds) <- specLayerIds) {
    ...
    val query = reader.query[GridTimeKey, Tile, TileLayerMetadata[GridTimeKey]](layerId)
