...
class TilerMethods[K, V <: CellGrid: ClassTag: (? => TileMergeMethods[V]): (? => TilePrototypeMethods[V])](val self: RDD[(K, V)]) extends MethodExtensions[RDD[(K, V)]] {
  ...
  def cutTiles[K2: GridComponent: ClassTag](rasterMetadata: TileLayerMetadata[K2], resampleMethod: ResampleMethod)
      (implicit ev: K => TilerKeyMethods[K, K2]): RDD[(K2, V)] =
    cutTiles(rasterMetadata.cellType, rasterMetadata.layout, resampleMethod)

  def cutTiles[K2: GridComponent: ClassTag](rasterMetadata: TileLayerMetadata[K2])
      (implicit ev: K => TilerKeyMethods[K, K2]): RDD[(K2, V)] =
  ...
  def tileToLayout[K2: GridComponent: ClassTag](rasterMetadata: TileLayerMetadata[K2], options: Options)
      (implicit ev: K => TilerKeyMethods[K, K2]): RDD[(K2, V)] =
    tileToLayout(rasterMetadata.cellType, rasterMetadata.layout, options)

  def tileToLayout[K2: GridComponent: ClassTag](rasterMetadata: TileLayerMetadata[K2])
      (implicit ev: K => TilerKeyMethods[K, K2]): RDD[(K2, V)] =
