...
object HadoopLayerHeader {
  implicit object HadoopTileLayerMetadataFormat extends RootJsonFormat[HadoopLayerHeader] {
      ...
      value.asJsObject.getFields("keyClass", "valueClass", "path") match {
        case Seq(JsString(keyClass), JsString(valueClass), JsString(path)) =>
          HadoopLayerHeader(
            ...
            new Path(path))
        case _ =>
          throw new DeserializationException(s"HadoopTileLayerMetadata expected, got: $value")
      }
  }
}
